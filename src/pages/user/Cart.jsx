...
export default function Cart() {
  ...
  return (
    <UserLayout>
      <div className="min-h-[85vh] bg-slate-50 font-sans py-10 relative overflow-hidden">
        ...
        <div className="max-w-[1240px] mx-auto px-4 sm:px-6 relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="lg:col-span-2 bg-white border border-[#d9c3ac] shadow-md rounded-2xl p-5 sm:p-7 text-left">
              <div className="flex items-center justify-between pb-5 border-b border-slate-100 mb-6">
                <div>
                  <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight">Your Cart</h1>
                ...
                {cartItems.length > 0 && (
                  <button 
                    ...
                    className="text-xs font-bold text-rose-600 hover:text-rose-700 bg-rose-50 hover:bg-rose-100/80 px-3 py-1.5 rounded-lg border border-rose-100/50 transition-all cursor-pointer"
                  >
                    Empty Cart
                  </button>
                )}
              ...
              {cartItems.length === 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <Link 
                    to="/" 
                    className="mt-6 inline-flex items-center gap-2 px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold transition-all shadow-md active:scale-[0.98] cursor-pointer"
                  >
                  ...
                  {cartItems.map((item) => (
                    <div key={item.id} className="py-4 first:pt-0 last:pb-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="flex items-start gap-3 min-w-0 flex-1">
                        <span className={`w-3 h-3 border shrink-0 flex items-center justify-center p-[2px] mt-1.5 ${item.veg ? 'border-green-600' : 'border-red-600'}`}>
                          <span className={`w-full h-full rounded-full ${item.veg ? 'bg-green-600' : 'bg-red-600'}`}></span>
                        </span>
                        <div className="min-w-0">
                          <h4 className="font-bold text-slate-800 text-sm sm:text-base truncate leading-snug">{item.name}</h4>
                          <div className="flex items-center gap-3 mt-1">
                            <span className="text-xs text-orange-500 font-bold">{item.price}</span>
                            <span className="text-[10px] text-slate-400 font-semibold">• Preparation time: 15-20 min</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between sm:justify-end gap-6 shrink-0">
                        ...
                        <div className="flex items-center gap-2.5 bg-slate-50 border border-slate-200/60 rounded-xl p-1">
                          <button 
                            ...
                            className="w-7 h-7 flex items-center justify-center text-slate-600 font-bold hover:bg-slate-200 rounded-lg text-sm transition-all cursor-pointer"
                          >
                            -
                          </button>
                          <span className="w-6 text-center font-extrabold text-xs text-slate-800">{item.qty}</span>
                          <button 
                        ...
                        <div className="w-20 text-right">
                          <span className="font-extrabold text-[#855400] text-sm sm:text-base">
            ...
            {cartItems.length > 0 && (
              <aside className="lg:col-span-1 space-y-6 w-full">
                ...
                {/* Bill details card */}
                <div className="bg-white border border-[#d9c3ac] shadow-md rounded-2xl p-6 text-left relative overflow-hidden">
                  {/* Premium visual top border */}
                  <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-orange-500 to-amber-500" />
                  
                  <h3 className="font-extrabold text-slate-800 text-base mb-5 flex items-center gap-2 border-b border-gray-100 pb-3 mt-1">
                  ...
                  <div className="space-y-3.5 text-xs font-semibold text-gray-500">
                    <div className="flex justify-between items-center">
                      <span>Subtotal</span>
                      <span className="font-bold text-gray-800 text-sm">₹{subtotal.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>GST (5%)</span>
                      <span className="font-bold text-gray-800 text-sm">₹{gst.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>Service Charge (5%)</span>
                      <span className="font-bold text-gray-800 text-sm">₹{serviceCharge.toLocaleString()}</span>
                    </div>
                    <hr className="my-4 border-dashed border-slate-200" />
                    <div className="flex justify-between items-end text-slate-900">
                      <div>
                        <span className="text-sm font-extrabold block text-gray-800">Total Payable</span>
                        <span className="text-[10px] text-gray-400 font-semibold">(incl. all taxes)</span>
                      </div>
                  ...
                  <button 
                    ...
                    className="w-full mt-6 py-3.5 bg-gradient-to-r from-[#FFA500] to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white rounded-xl font-bold text-sm sm:text-base transition-all shadow-md active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2"
                  >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-[460px] overflow-hidden border border-[#d9c3ac] shadow-2xl flex flex-col p-6 text-center">
            ...
            <div className="bg-[#fff8ed] border border-[#d9c3ac]/40 rounded-xl p-4 my-5 text-left">
              <h4 className="font-bold text-[#855400] text-xs uppercase tracking-wider mb-2">Order Summary</h4>
