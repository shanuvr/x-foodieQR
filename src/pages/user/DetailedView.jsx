...
export default function DetailedView() {
  ...
  const firstStickyRef = useRef(null);
  const [firstStickyHeight, setFirstStickyHeight] = useState(120);

  useEffect(() => {
    const handleResize = () => {
      if (firstStickyRef.current) {
        const rect = firstStickyRef.current.getBoundingClientRect();
        setFirstStickyHeight(rect.height);
      }
    };
    ...
    let observer;
    if (firstStickyRef.current && window.ResizeObserver) {
  ...
  return (
    <UserLayout>
      <div className="max-w-[1240px] mx-auto px-4 sm:px-6 py-6 font-sans">
        ...
        <div 
          style={{ top: `calc(var(--navbar-height, 80px) + ${firstStickyHeight}px)` }}
          className="sticky z-30 bg-white/95 backdrop-blur-md -mx-4 sm:-mx-6 px-4 sm:px-6 md:px-8 border-y border-[#d9c3ac]/60 sm:border rounded-none sm:rounded-2xl py-2 sm:py-3.5 text-left mb-2 flex flex-row items-center justify-between gap-3 relative overflow-hidden transition-[top] duration-300 ease-in-out"
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mt-3">
          <div className="lg:col-span-2">
            ...
            <section className="py-6 sm:py-12 border-t border-[#d9c3ac] mt-6 sm:mt-12" id="overview">
              <h2 className="font-bold text-[18px] sm:text-[24px] text-[#212529] mb-3 sm:mb-6">About Us</h2>
              <p className="text-sm sm:text-[16px] text-[#534433] leading-relaxed">Welcome to {profile.kitchenName || 'Novotel Signature Restaurant'}, where culinary art meets timeless hospitality in the heart of Hyderabad. Our kitchen is led by award-winning chefs dedicated to crafting an exceptional dining experience. We blend authentic local heritage with contemporary global cooking techniques, selecting only the finest organic produce and sustainably sourced ingredients. Whether you are hosting a celebratory family dinner or a sophisticated business lunch, our warm ambience and curated menus promise to make every visit truly memorable.</p>

              {/* Dynamic Website & Instagram Links */}
              <div className="flex flex-wrap gap-2.5 sm:gap-4 mt-4 sm:mt-6">
                {profile.website && (
                  <a 
                    href={profile.website} 
                    target="_blank" 
                    rel="noreferrer" 
                    className="inline-flex items-center gap-1.5 sm:gap-2.5 px-3.5 py-2 sm:px-5 sm:py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-bold shadow-md shadow-orange-500/10 transition-colors cursor-pointer"
                  >
                ...
                {profile.instagram && (
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer" 
                    className="inline-flex items-center gap-1.5 sm:gap-2.5 px-3.5 py-2 sm:px-5 sm:py-2.5 border border-slate-200 hover:border-slate-300 text-slate-700 bg-white rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-bold shadow-sm transition-colors cursor-pointer"
                  >
                    <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-pink-600" fill="currentColor" viewBox="0 0 24 24">
